...
public interface IAdvtReportService
{
    ...
    public Task<AdvtReportDto> GetAdvtReportById(int advtReportId);
    ...
    /// <returns>Модель представления жалобы <see cref="AdvtReportDto"/></returns>
    public Task<AdvtReportDto> CreateAdvtReport(AdvtReportDto advtReportDto);
    ...
    public Task<IEnumerable<AdvtReportDto>> GetAllAdvtReports(AdvtReportFilterRequest? filterRequest);

    /// <summary>
    /// Удаляет жалобу.
    /// </summary>
    /// <param name="advtReportId">Идентификатор жалобы.</param>
    public Task DeleteAdvtReport(int advtReportId);
    ...
    public Task UpdateAdvtReport(int advtReportId, AdvtReportDto advtReportDto);
}